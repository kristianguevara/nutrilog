...
function logStep(requestId: string, line: string): void {
  console.log(`[macro-estimate ${requestId.slice(0, 8)}] ${line}`);
}
...
export default async function handler(req: VercelRequest, res: VercelResponse): Promise<void> {
  ...
  try {
    ...
    const parsed = foodMacroEstimateRequestSchema.safeParse(rawBody);
    if (!parsed.success) {
      sendJsonError(res, 400, requestId, "INVALID_BODY", "Invalid request body", parsed.error.flatten());
      return;
    }

    const result = await runMacroEstimate(parsed.data, { requestId, handlerStartedAt, logger });

    if (!result.ok) {
      const { code, message, details } = result;
      if (code === "MISSING_OPENAI_KEY" || code === "MISSING_GEMINI_KEY") {
    ...
    const message = e instanceof Error ? e.message : "Unexpected error in food-macro-estimate handler";
